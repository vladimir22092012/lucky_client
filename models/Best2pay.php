...
class Best2pay extends Core
{
    ...
    Личный кабинет
    https://pay.best2pay.net/office/
    login: nalichnoeplus.com
    password: 4U40li7LsP0s1
    Sector ID: 7179 ООО МКК "На личное+" (nalichnoeplus.com) (ecom) (СНГБ)
    Sector ID: 7180 ООО МКК "На личное+" (nalichnoeplus.com) (P2PCredit) (СНГБ) 
    Sector ID: 7181 ООО МКК "На личное+" (nalichnoeplus.com) (token) (СНГБ) 
    Sector ID: 7182 ООО МКК "На личное+" (nalichnoeplus.com) (C2A) (ФЛ) (СНГБ) 
    Sector ID: 7184 ООО МКК "На личное+" (nalichnoeplus.com) (C2A) (ЮЛ) (СНГБ)

    7179 используем для любого дергания денег с карты без 3дс (услуги, просрочка). 
        Используем данный сектор когда нет первой операции с 3дс, 
        на которую мы прикрутили рекаринг период равный 0.

    7180 - выдача 
    
    7181 сектор для токенизации карты- его используем если авторайз+реверс. 
        Больше он не подойдёт ни для чего 
    
    7182 - погашение. Тут операции с 3дс. Fee передаём на этот сектор 1,8% минимум 30 рублей. 
        Реккаринг период=0.
    
    7184 - погашение. Будем использовать в первоначальном плане. 
        Т.е списываем безвозвратно 1 рубль. Реккаринг период равный 0. Fee=0
        При попытке дальнейшего списания безакцептного списания обращаемся к этим заказам.

нужно только заменить сектора
МИНБ -> СНГБ
...
7182 -> 8305 (C2A) (ФЛ) - для оплаты займа
    ...
    */
    ...
    private $sectors = array(
        ...
        'PAYMENT' => '4220', // сектор для оплаты любой картой
        //'YUK' => '7036', // подменный кабинет
        //'PREMIER' => '7864', // подменный кабинет Премьер
        //'TOROS' => '8919', //подменный кабинет Торос
    );
...
//        '8919' => 'nv953BGI770', //подменный кабинет Торос
//
//        '8303' => 'H16phg0',
//        '8304' => '5aF52ladm0',
          '4221' => 'test'
