...
class SmsCode extends Core
{
    ...
    private function send_accept_code_action($phone)
    {
        ...
        {            
            ...
            if ($contract = $this->contracts->get_contract($contract_id) && $contract->try_accept_code < 3)
            {
                $msg = 'Активируй займ '.($contract->amount*1).' в личном кабинете, код '.$contract->accept_code.' '.$this->config->main_domain.'/lk';
            ...
            if($contract->try_accept_code > 3)
            {
                $this->response['error'] = 'Для повторного получения кода, обратись в колл-центр по бесплатному номеру 8-800-444-8234';
            }
            else
            {
                $this->response['error'] = 'Договор не найден!';
    ...
    private function send_action($phone)
    {
        ...
        else
        {
            if ($phone == '7000 000-0011')
                $rand_code = '0000';
            elseif ($phone == '7963 914-3424')
                $rand_code = '3429';
            elseif ($phone == '7913 909-6454')
                $rand_code = '4546';
            else
                $rand_code = mt_rand(1000, 9999);
